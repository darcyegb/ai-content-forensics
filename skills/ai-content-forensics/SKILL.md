...
- **Gemini API via Vertex AI** (`pip install google-genai google-auth`) — Analyzes actual video content directly from YouTube URLs. Examines set design, camera work, on-screen graphics, B-roll, visual branding, and production quality. Production-tested: processes ~2 min per video, outputs 7-10K chars of detailed analysis per video. **CRITICAL AUTH**: YouTube video analysis requires the **Vertex AI path with OAuth credentials** from Gemini CLI (`~/.gemini/oauth_creds.json`), NOT a plain API key. A plain API key returns 403 on YouTube video features. Set `GOOGLE_CLOUD_PROJECT=creator-ui` (the project NAME, not the numeric ID). Always set `max_output_tokens=65536` — the default (8192) silently truncates output. See `references/phase1_research.md` for the full auth setup and code.
...
- **Gemini video analysis**: Requires Vertex AI path with OAuth credentials from `~/.gemini/oauth_creds.json` (from Gemini CLI sign-in). Set `GOOGLE_CLOUD_PROJECT=creator-ui`. Do NOT use a plain API key for YouTube video analysis — it will 403. See `references/phase1_research.md` for the auth setup. **CRITICAL: Always set `max_output_tokens=65536`** — the default (8192) silently truncates output.
